"""
juiz_llm_gemini.py — LLM como juiz (Gemini) para os datasets de comparação A/B.

Versão adaptada do juiz_llm.py original: troca o motor Ollama/Llama 3.1 8B pelo
Gemini (Google) via SDK oficial `google-genai`. Como o feedback foi GERADO pelo
Llama, usar um juiz de família diferente (Gemini) mitiga o viés de
...
Observações:
  ...
  - temperature=0.0 + saída forçada em JSON (response_mime_type/schema) para
    máxima reprodutibilidade e parsing confiável.

REQUISITOS:
    pip install google-genai pandas
    export GEMINI_API_KEY="sua_chave"     # pegue no Google AI Studio (tier grátis)

Uso:
    python juiz_llm_gemini.py                 # julga dataset_c1..c5.csv na pasta atual
    python juiz_llm_gemini.py .               # idem, pasta explícita
    python juiz_llm_gemini.py datasets/       # julga os dataset_c*.csv dentro de datasets/
    python juiz_llm_gemini.py datasets/ 3     # julga apenas o dataset_c3.csv
"""
import os
import re
import sys
...
# Modelo do juiz. Opções comuns no tier gratuito:
#   "gemini-2.5-flash"  -> rápido e barato, ótimos limites grátis (padrão)
#   "gemini-2.5-pro"    -> mais capaz, melhor para julgar nuances (limites menores)
# Verifique no Google AI Studio qual versão está disponível para sua chave.
LLM = "gemini-2.5-flash"

# Controle de taxa (tier gratuito tem limite de requisições por minuto).
PAUSA_ENTRE_CHAMADAS = 1.0   # segundos entre uma linha e outra
MAX_TENTATIVAS = 5           # re-tentativas com backoff em caso de erro/limite

# Cliente Gemini. A chave é lida da variável de ambiente GEMINI_API_KEY.
if not os.environ.get("GEMINI_API_KEY"):
    sys.exit("[!] Defina a variável de ambiente GEMINI_API_KEY antes de rodar.")
client = genai.Client()
...
# Schema que força a saída a ser exatamente {"escolha": "A"|"B"}.
# Se sua versão do SDK reclamar do formato, basta remover `response_schema`
# abaixo; o `response_mime_type="application/json"` + o parser já dão conta.
RESPONSE_SCHEMA = {
    "type": "OBJECT",
    "properties": {"escolha": {"type": "STRING", "enum": ["A", "B"]}},
    "required": ["escolha"],
}
...
def julgar(prompt, essay, n, fb_a, fb_b):
    """Chama o Gemini com re-tentativas e backoff exponencial."""
    conteudo = montar_user(prompt, essay, n, fb_a, fb_b)
    ultimo_erro = None
    for tentativa in range(MAX_TENTATIVAS):
        try:
            ...
            return _parse_escolha(resp.text)
        except Exception as e:                              # noqa: BLE001
            ultimo_erro = e
            espera = 2 ** tentativa                         # 1, 2, 4, 8, 16s...
...
def processar_dataset(caminho, n):
    ...
    for i, linha in df.iterrows():
        ...
        try:
            ...
            print(f"[{i + 1}/{total}] ERRO após retentativas: {e} — pulando.")
            continue
        ...
        time.sleep(PAUSA_ENTRE_CHAMADAS)                    # respeita limite de RPM
